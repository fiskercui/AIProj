...
namespace TMPro
{
    ...
    public partial class TextMeshProUGUI
    {
        ...
        static private Matrix4x4 m_EnvMapMatrix = new Matrix4x4();
        ...
        protected override void Awake()
        {
            ...
            if (m_mesh == null)
            {
                ...
                m_mesh = TMP_SubMesh.NewMesh();// new Mesh();
                m_mesh.hideFlags = HideFlags.HideAndDontSave;
                #if DEVELOPMENT_BUILD || UNITY_EDITOR
                m_mesh.name = "TextMeshPro UI Mesh";
                #endif
                // Create new TextInfo for the text object.
                m_textInfo = TMP_TextInfo.AlocTextInfo(this); //new TMP_TextInfo(this);
            }
            ...
            List<TMP_SubMeshUI> subTextObjects = ListPool<TMP_SubMeshUI>.Get();
            GetComponentsInChildren<TMP_SubMeshUI>(subTextObjects);
            if (subTextObjects.Count > 0)
            {
                int subTextObjectCount = subTextObjects.Count;
            ...
            ListPool<TMP_SubMeshUI>.Release(subTextObjects);
        ...
        protected override void OnDisable()
        {
            ...
            {
                Debug.LogError("跳过 layout=" + m_rectTransform.name);
            }
            */
        ...
        protected override void OnDestroy()
        {
            ...
            if (m_mesh != null)
            {
                TMP_SubMesh.RecycleMesh(m_mesh);
                m_mesh = null;
            ...
            TMP_TextInfo.RecycleTextInfo(m_textInfo);
            m_textInfo = null;
        }
        ...
        void OnPrefabInstanceUpdate(GameObject go)
        {
            ...
            if (go == this.gameObject)
            {
                ...
                if (subTextObjects.Count > 0)
                {
                    for (int i = 0; i < subTextObjects.Count; i++)
                        m_subTextObjects[i + 1] = subTextObjects[i];
                }
        ...
        /// </summary>
        private Canvas GetCanvas(bool igreoActive = false)
        {
        ...
        internal override bool IsMaterialChange()
        {
            ...
            //需要重新创建mesh info
            if (materialCount > m_textInfo.meshInfo.Length|| materialCount > m_subTextObjects.Length)
            {
                return true;
            }
            ...
            {
                if (m_textInfo.characterInfo[i].materialReferenceIndex > materialCount)
                {
                    materialCount = m_textInfo.characterInfo[i].materialReferenceIndex;
                }
            }
            ...
            for (int i = 1; i < materialCount; i++)
            {
                ...
                if (i > 0)
                {
                    ...
                    if (m_materialReferences[i].isFallbackMaterial && m_subTextObjects[i].fallbackMaterial == null)
                    {
                        return true;
                    }
                }
        ...
        internal override int SetArraySizes(UnicodeChar[] unicodeChars)
        {
            ...
            if (m_textInfo == null)
            {
                m_textInfo = TMP_TextInfo.AlocTextInfo(this);
                //m_textInfo = new TMP_TextInfo(m_InternalTextProcessingArraySize);
            }


            if (m_textInfo.characterInfo.Length < m_InternalTextProcessingArraySize)
            ...
            for (int i = 0; i < materialCount; i++)
            {
                ...
                if (m_textInfo.meshInfo[i].vertices == null || m_textInfo.meshInfo[i].vertices.Length < referenceCount * 4)
                {
                    if (m_textInfo.meshInfo[i].vertices == null)
                    {
                        ...
                        if (i == 0)
                        {
                            m_textInfo.meshInfo[i].Rebuild(m_mesh, referenceCount + 1);
                            //m_textInfo.meshInfo[i] = new TMP_MeshInfo(m_mesh, referenceCount + 1);
                        }
                        else
                        {
                            m_textInfo.meshInfo[i].Rebuild(m_subTextObjects[i].mesh, referenceCount + 1);
        ...
        public override void ComputeMarginSize()
        {
            if (this.rectTransform != null)
            {
                ...
                m_cornerNeedReCalc = true;

                /*
                if(m_cornerNeedReCalc)
                {
                    Debug.LogError("repeate calc m_cornerNeedReCalc hashCode = "+GetHashCode() + " name="+name);
                }
                */
