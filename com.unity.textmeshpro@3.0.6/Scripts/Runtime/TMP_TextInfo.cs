...
namespace TMPro
{
    ...
    public class TMP_TextInfo
    {
        ...
        private static bool s_InitTextInfo = false;
        public TMP_TextInfo(TMP_Text textComponent)
        {
            //初始化的时候，创建256个
            if (s_InitTextInfo == false)
            {
                s_InitTextInfo = true;
                for (int i = 0; i < 512; ++i)
                {
                    RecycleTextInfo(new TMP_TextInfo());
                }
            }
        ...
        public void ReInit(TMP_Text textComponent)
        {
            //Clear();
            this.textComponent = textComponent;
            meshInfo[0].mesh = textComponent.mesh;
        ...
        public void Reset()
        {
            for(int i=0;i< characterCount&&i<characterInfo.Length;++i)
            {
                characterInfo[i].Reset();
            }

            for (int i = 0; i < wordCount&&i< wordInfo.Length; ++i)
            {
                wordInfo[i].Reset();
            }

            for (int i = 0; i < linkCount&&i<linkInfo.Length; ++i)
            {
                linkInfo[i].Reset();
            }

            for (int i = 0; i < lineCount&&i< lineInfo.Length; ++i)
            {
                lineInfo[i].Reset();
            }

            for (int i = 0; i < pageCount&&i< pageInfo.Length; ++i)
            {
                pageInfo[i].Reset();
            }
        ...
        static Stack<TMP_TextInfo> s_TMPPool = new Stack<TMP_TextInfo>();
        static public TMP_TextInfo AlocTextInfo(TMP_Text textComponent)
        {
            int nCount = s_TMPPool.Count;
            if (nCount > 0)
            {
#if UNITY_EDITOR
                if(nCount>2048)
                {
                    Debug.LogError("可能存在内存泄露 TMP_TextInfo count = " + nCount) ;
                }
#endif
                TMP_TextInfo ti = s_TMPPool.Pop();
                ti.ReInit(textComponent);
                return ti;
            }
        ...
        static public void RecycleTextInfo(TMP_TextInfo ti)
        {
            if (null != ti)
            {
                ti.Reset();
